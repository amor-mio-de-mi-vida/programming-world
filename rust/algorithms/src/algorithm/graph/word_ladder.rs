...
fn build_word_graph(words: Vec<&str>) -> Graph<String> {
    ...
    for word in words{
        for i in 0..word.len() {
            ...
            if d.contains_key(&pattn) {
                d.get_mut(&pattn).unwrap().push(wd);
            } else {
                d.insert(pattn, vec![wd]);
            }
        }
    }
...
fn word_ladder(g:&mut Graph<String>, start: Vertex<String>, end: Vertex<String>, len: usize) -> u32{
    ...
    while vertex_queue.len() > 0 {
        // 节点出队
        let mut curr = vertex_queue.dequeue().unwrap();
        for nbr in curr.get_neighbors() {
            // 克隆， 避免和图中数据起冲突
            // Graph 的vertices用RefCell包裹就不用克隆
            let mut nbv = g.vertices.get_mut(nbr).unwrap().clone();
            ...
            if end.key != nbv.key {
                ...
                if Color::White == nbv.color {
                    ...
                    // 图中的节点也需要更新颜色和距离 (因为nbv是clone版本的)
